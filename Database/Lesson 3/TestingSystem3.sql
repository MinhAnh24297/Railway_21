...
CREATE TABLE `Account`(
	...
	DepartmentID 		MEDIUMINT UNSIGNED NOT NULL REFERENCES Department(DepartmentID),
	PositionID 			MEDIUMINT UNSIGNED NOT NULL REFERENCES `Position`(PositionID),
...
CREATE TABLE GroupAccount(
	GroupID 			MEDIUMINT UNSIGNED NOT NULL REFERENCES `Group`(GroupID),
...
CREATE TABLE Question(
	...
	CategoryID 			MEDIUMINT UNSIGNED NOT NULL REFERENCES CategoryQuestion(CategoryID),
	TypeID 				MEDIUMINT UNSIGNED NOT NULL REFERENCES TypeQuestion(TypeID),
	CreatorID 			MEDIUMINT UNSIGNED NOT NULL REFERENCES `Account`(AccountId),
	CreateDate 			DATETIME DEFAULT NOW()
);
...
CREATE TABLE ExamQuestion(
	ExamID 				MEDIUMINT UNSIGNED NOT NULL REFERENCES Exam(ExamID),
	QuestionID 			MEDIUMINT UNSIGNED NOT NULL REFERENCES Question(QuestionID) ON DELETE CASCADE,
...
-- Insert data into table 5: GroupAccount
...
-- Insert data into table 6: TypeQuestion
INSERT INTO TypeQuestion (TypeName )
VALUES 	('Essay' ),		
		('Multiple-Choice' );
        
-- Insert data into table 7: CategoryQuestion
...
insert into Answer (Content, QuestionID, isCorrect) values ('Answers VTI 1898', 174, 0);
...
-- Insert data into table 11: ExamQuestion
...
insert into ExamQuestion (ExamID, QuestionID) values (8, 54);
insert into ExamQuestion (ExamID, QuestionID) values (5, 196);
...
insert into ExamQuestion (ExamID, QuestionID) values (6, 42);
insert into ExamQuestion (ExamID, QuestionID) values (9, 123);
...
insert into ExamQuestion (ExamID, QuestionID) values (7, 76);
insert into ExamQuestion (ExamID, QuestionID) values (1, 167);
...
insert into ExamQuestion (ExamID, QuestionID) values (10, 242);
insert into ExamQuestion (ExamID, QuestionID) values (6, 249);
...
insert into ExamQuestion (ExamID, QuestionID) values (10, 88);
...
-- Q8: Lấy ra các mã đề thi có thời gian thi >= 60 phút và được tạo trước ngày 20/12/2012
SELECT `Code`
FROM exam
...
-- Q10: Đếm số nhân viên của department có ID = 2
SELECT 	COUNT(AccountID) AS 'SoNhanViên'
FROM 	`Account`
WHERE DepartmentID = 2;

-- Q11: Lấy ra nhân viên có tên bắt đầu bằng chữ "D" và kết thúc bằng chữ "r"
SELECT 	*
FROM 	`Account`
WHERE FullName LIKE 'D%r';
...
-- Q13: Xóa tất cả các question có nội dung bắt đầu bằng từ "Question"
SET SQL_SAFE_UPDATES = 0;
DELETE
FROM question
